...
class corpus(dict):
    ...
    def get_sentences(self):
        """Builds a sentence frequency table.
        ...
        my_sentences = []
        for doc in self.get_documents():
            my_sentences.extend([sentence for sentence in doc.sents])
        return my_sentences
    ...
    def get_sentence_level_sentiment(self):
        """Gets sentiment on the sentence level
        :returns: a list of pairs (item, sentiment)
        :rtype: list
        """
        sentence_sentiments = []
        sentence_counts = self.get_sentences()
        for sentence in sentence_counts:
            sentence_sentiments.append((str(sentence), corpus.sentence_sentiment(str(sentence))[0]['label']))
        return sentence_sentiments

    def render_doc_sentiments(self, doc_id):
        """Gets sentiment on the sentence level
        :param doc_id: the id of a spaCy doc made from the text in the document
        :type doc: str
        :returns: a list of pairs (item, frequency)
        ...
        doc = self.get_document(doc_id)
        # Same definition as in project 3b, but prefix the output file name with self.name to make it unique to this corpus
        # make the sentence table
        sentence_table = "| Sentence | Label |\n| ------ | ------ | \n"
        # walk over the sentences in the document
        for sentence in doc.sents:
            sentence_table  = sentence_table + "| " + str(sentence) + " | " + str(corpus.sentence_sentiment(str(sentence))[0]['label']) + " |\n"
        # make the document last line
        print(doc)
        sentence_table  = sentence_table + "| " + str(doc) + " | " + str(self[doc_id]['sentiment-analysis']['label']) + " |\n"
        return '## Sentiments\n' + sentence_table
